    ...
    while (not resuelto) and len(nodos_frontera) != 0:
        ...
        if nodo_actual.get_estado() == solucion:
            ...
            for i in range(0, numero+1):
              ...
              temp=estado_nodo[i]
              estado_nodo[i]=estado_nodo[i+1]
              estado_nodo[i+1] = temp
            nuevo_hijo=Nodo(estado_nodo)
            nodo_actual.set_hijo(nuevo_hijo)
            nuevo_hijo.set_padre(nodo_actual)
            # operador izquierdo
...
if __name__ == "__main__":
    lista_final=[]
    numero = int(input('Introduzca tamaño de rompecabezas: '))
    for i in range(0, numero):
       lista_final.append(i);
    
    estado = sorted(lista_final, reverse=True)
    estado_inicial=estado
    solucion = lista_final
